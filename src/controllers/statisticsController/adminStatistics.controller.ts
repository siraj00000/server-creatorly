...
function splitRevenueDataByDays(data: any[], startDate: Date, endDate: Date, numberOfDays: number): any[] {
    const newData: any[] = [];
...
class AdminStatisticsController {
    public async analytics(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const [creatorUsersCount, brandAgencyUsersCount]: any = await Promise.all([
            ...
            const [singleCreatorCampaignData, multiCreatorCampaignData]: any = await Promise.all([
    ...
    //
    public async siteAnalytics(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const month = req.params.month; // Assuming month is in the format 'MM'
            const year = new Date().getFullYear(); // Get the current year
            const startDate = moment(`${month}-${year}`, 'MM-YYYY').startOf('month').toDate(); // Get the start date of the month
            ...
            // Count completed single/multi-creator campaigns in the month
            const [singleCreatorCampaignCount, multiCreatorCampaignCount]: any = await Promise.all([
            ...
            const data = {
                ...
                multiCreatorCampaignCount,
                totalCompletedCampaignCount:
                    (singleCreatorCampaignCount && singleCreatorCampaignCount[0] ? singleCreatorCampaignCount[0].count : 0) +
                    (multiCreatorCampaignCount && multiCreatorCampaignCount[0] ? multiCreatorCampaignCount[0].count : 0),
                totalBudget:
                    (singleCreatorCampaignCount && singleCreatorCampaignCount[0] ? singleCreatorCampaignCount[0].budget : 0) +
    ...
    public async getStats(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const [creatorUsersCount, brandAgencyUsersCount, singleCreatorCampaignCount, multiCreatorCampaignCount]: any = await Promise.all([
