...
import DetailInvoice from '../../models/creatorModels/invoiceModel/detailInvoice.model.js';
...
class InvoiceController {
    async createInvoice(req, res, next) {
        try {
            ...
            const detailInvoicePromises = detailInvoices.map(async (detailInvoiceData) => {
    ...
    async getMasterInvoices(req, res, next) {
        try {
            // Fetch the master invoice
            const masterInvoice = await MasterInvoice.find({ user_id: req.user?.id }, { 'to.Name': 1, date: 1, due: 1, invoice_number: 1, 'total.balance_due': 1 });
